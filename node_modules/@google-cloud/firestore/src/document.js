 ...
 */
DocumentSnapshot.Builder = function(snapshot) {
  var builder = {};
   ...
   * @type {object}
   */
  builder.fieldsProto = snapshot.fieldsProto_;
   ...
   * @type {string}
   */
  builder.readTime = snapshot.readTime;
   ...
   * @type {string}
   */
  builder.createTime = snapshot.createTime;
   ...
   * @type {string}
   */
  builder.updateTime = snapshot.updateTime;
   ...
   */
  builder.build = () => {
    assert(is.defined(builder.fieldsProto) === is.defined(builder.createTime),
        'Create time should be set iff document exists.');
    assert(is.defined(builder.fieldsProto) === is.defined(builder.updateTime),
        'Update time should be set iff document exists.');
    return new DocumentSnapshot(builder.ref, builder.fieldsProto,
        builder.readTime, builder.createTime, builder.updateTime);
  };
 ...
 * the input is empty.
 */
DocumentSnapshot.toISOTime = function(timestamp) {
  if (timestamp) {
    var isoSubstring = new Date(timestamp.seconds * 1000).toISOString();
    ...
    var nanoString = timestamp.nanos + '';
 ...
 * @return {Object}
 */
DocumentMask.fromObject = function(data) {
  var fieldPaths = [];
...
Precondition.prototype.toProto = function() {
  ...
  } else if (is.defined(this.exists)) {
