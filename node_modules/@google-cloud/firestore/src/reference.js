...
DocumentReference.prototype.collection = function(collectionPath) {
  ...
  return new CollectionReference(this.firestore, path);
};
 ...
 * @return Promise.<firestore.WriteResult> A Promise that resolves once the data
 ...
 * @return Promise.<firestore.WriteResult> A Promise that resolves once the data
 * has been successfully written to the backend.
 *
 * @example
 * var documentRef = firestore.doc('col/doc');
 *
  ...
  return watch.onSnapshot(
    (readTime, docMap) => {
      if (docMap.has(this.formattedName)) {
        ...
        document.ref = this.referencePath_;
...
Query.prototype.where = function(fieldPath, opStr, value) {
  ...
  var newFilter = new FieldFilter(fieldPath, comparisonOperators[opStr], value);
 ...
 * });
 */
Query.prototype.select = function(varArgs) {
...
Query.prototype.orderBy = function(fieldPath, directionStr) {
  ...
  var newOrder = new FieldOrder(fieldPath, directionOperators[directionStr]);
 ...
 * ordered by fields a, b, c and finally by key, then if the fields contain c it
 * must also contain b and a; if they contain b, they must also contain a.
 *
 ...
 * c it must also contain b and a; if they contain b, they must also contain a.
 *
 * The ending position is relative to the order of the query.
 *
 * @public
...
Query.prototype.get_ = function(queryOptions) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.stream_(queryOptions)
      ...
      .on('end', () => {
        resolve(
            new QuerySnapshot(this, readTime, immutable.Map(docs), changes));
      });
  });
};
...
Query.prototype.stream = function() {
  var responseStream = this.stream_();
...
Query.prototype.stream_ = function(queryOptions) {
  ...
  var stream = through.obj(function(proto, enc, callback) {
    var readTime = DocumentSnapshot.toISOTime(proto.readTime);

    if (proto.document) {
      var response = proto.document;
      var document = DocumentSnapshot.Builder();
 ...
 * new [QuerySnapshot]{@link firestore.QuerySnapshot} is available.
 * @param {errorCallback=} onError - A callback to be called if the listen fails
...
Query.prototype.comparator = function() {
  return (doc1, doc2) => {
    ...
    let lastDirection = this.fieldOrders_.length === 0 ?
        directionOperators.ASC :
        this.fieldOrders_[this.fieldOrders_.length - 1].direction;
    let orderBys = this.fieldOrders_.concat(
        new FieldOrder(DOCUMENT_NAME_FIELD, lastDirection));

    for (let orderBy of orderBys) {
      let comp;
      if (orderBy.field === DOCUMENT_NAME_FIELD) {
        comp = Path.compare(doc1.ref.referencePath_, doc2.ref.referencePath_);
      } else {
 ...
 *
 * @alias firestore.DcumentChange
 * @hideconstructor
 *
 * @param {string} type - 'added' | 'removed' | 'modified'.
 * @param {firestore.DocumentSnapshot} snapshot - The document.
 */
 ...
 */
function QuerySnapshot(query, readTime, docMap, docChanges) {
  /**
  ...
  Object.defineProperty(this, 'docs', {
    get: () => {
      ...
      this.docs_ = docMap.toList().toJS();
      this.docs_.sort(this.comparator_);
      return this.docs_;
    }
  });
  ...
  Object.defineProperty(this, 'docChanges', {
    get: () => {
      if (this.docChanges_) {
        return this.docChanges_;
      }
      this.docChanges_ = docChanges;
      this.docChanges_.sort((c1, c2) => {
        var typeOrder = order.primitiveComparator(documentChangeOrder[c1.type],
            documentChangeOrder[c2.type]);
        return typeOrder || this.comparator_(c1.doc, c2.doc);
      });
      return this.docChanges_;
    }
  });
   ...
   * });
   */
  Object.defineProperty(this, 'readTime', {
    value: readTime,
    writable: false
  });

  this.comparator_ = this.query.comparator();
  this.docs_ = null;
  this.docChanges_ = null;
}
 ...
 * [DocumentSnapshot]{@link firestore.DocumentSnapshot} for each document in the
 * snapshot.
 *
 * @example
 * var query = firestore.collection('col').where('foo', '==', 'bar');
...
function FieldOrder(field, direction) {
   ...
   * @type string
   */
  this.direction = direction || directionOperators.ASC;
}
 ...
 * [DocumentReference]{@link firestore.DocumentReference} pointing to the newly
 * created document.
 *
 * @example
 * var collectionRef = firestore.collection('col');
...
module.exports = (FirestoreType) => {
  ...
  return {
    ...
    validateDocumentReference: validateDocumentReference
  };
};
