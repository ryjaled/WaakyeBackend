 ...
 * @param {Object.<string,firestore.DocumentChange>} changeMap - A map
...
Watch.prototype.onSnapshot = function(onNext, onError) {
  ...
  const resetDocs = function() {
    ...
    docMap.forEach((value, key) => {
      // Mark each document as deleted. If documents are not deleted, they will
      // be send again by the server.
      changeSet[key] = REMOVED;
    });
  ...
  const computeChanges = function(mutableMap, mutableChanges, readTime) {
    ...
    for (let name in changeSet) {
      if (changeSet.hasOwnProperty(name)) {
        let oldDocument = mutableMap.get(name);
        let newBuilder = changeSet[name];

        if (newBuilder === REMOVED) {
          ...
          newBuilder.readTime = readTime;
          let newDocument = newBuilder.build();
  ...
  const push = function(readTime) {
    ...
    if (!hasPushed || Object.keys(changes).length) {
  ...
  /**
   * Returns the current count of all documents, including the changes from the
   * current changeSet.
   */
  ...
  stream.on('data', proto => {
    if (proto.targetChange) {
      ...
      if (changed) {
        ...
        changeSet[name] = snapshot;
      } else if (removed) {
        changeSet[name] = REMOVED;
      }
