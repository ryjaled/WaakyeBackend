...
var DocumentReference = require('../src/reference').DocumentReference;
var document = require('../src/document')(DocumentReference);
...
describe('Firestore class', function() {
  ...
  it('has collection() method', function() {
    var ref = firestore.collection('col');
    assert.equal(ref.id, 'col');
  });

  it('has doc() method', function() {
    var ref = firestore.doc('col/doc');
    assert.equal(ref.id, 'doc');
  });

  it('has getAll() method', function() {
    var ref1 = randomCol.doc('doc1');
    var ref2 = randomCol.doc('doc2');
    return Promise.all([
      ref1.set({foo: 'a'}),
      ref2.set({foo: 'a'})]).then(() => {
...
describe('CollectionReference class', function() {
  ...
  it('has parent property', function() {
    var ref = firestore.collection('col/doc/col');
  ...
  it('has doc() method', function() {
    var ref = firestore.collection('col').doc('doc');
    assert.equal(ref.id, 'doc');
...
describe('DocumentReference class', function() {
  ...
  it('has collection() method', function() {
    var ref = firestore.doc('col/doc').collection('subcol');
    assert.equal(ref.id, 'subcol');
  });

  it('has create()/get() method', function() {
    var ref = randomCol.doc();
  ...
  it('can delete() a non-existing document', function() {
    var ref = firestore.collection('col').doc();
    return ref.delete();
  });
  ...
  describe('watch', function() {
    var currentDeferred = { promise: null };
    ...
    it('handles changing a doc', function() {
      var ref = randomCol.doc('doc');
      var readTime, createTime, updateTime;
    ...
    it('handles multiple docs', function(done) {
      var doc1 = randomCol.doc();
      var doc2 = randomCol.doc();

      var unsubscribe1, unsubscribe2;

      // Documents transition from non-existent to existent to non-existent.
      var exists1 = [ false, true, false ];
      var exists2 = [ false, true, false ];
...
describe('Query class', function() {
  ...
  describe('watch', function() {
    ...
    const snapshot = function(id, data) {
      ...
      var snapshot = DocumentSnapshot.Builder();
      snapshot.ref = ref;
    ...
    it('handles changing a doc', function() {
      ...
      var unsubscribe = randomCol.onSnapshot((snapshot) => {
        currentDeferred.resolve(snapshot);
      }, (err) => {
        currentDeferred.reject(err);
      });
    ...
    it('handles changing a doc so it doesn\'t match', function() {
      ...
      var query = randomCol.where('included', '==', 'yes');
...
describe('Transaction class', function() {
  ...
  it('has get() with query', function() {
    var ref = randomCol.doc('doc');
    var query = randomCol.where('foo', '==', 'bar');
    return ref.set({foo: 'bar'}).then(() => {
      return firestore.runTransaction(updateFunction => {
...
describe('QuerySnapshot class', function() {
  ...
  beforeEach(function() {
    ...
    var randomCol = getTestRoot(firestore);
    var ref1 = randomCol.doc('doc1');
    var ref2 = randomCol.doc('doc2');
